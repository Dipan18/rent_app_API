...
class Products {
  ...
  public function get_products() {
      ...
      $query =   'select products.pro_id, pro_title, pro_price, img_name, img_path
                  from product_images 
                  inner join products
      ...
      $statement = $this->conn->prepare($query);
      $statement->bindParam(':limit', $per_page, PDO::PARAM_INT);
      $statement->bindParam(':offset', $row_start_index, PDO::PARAM_INT);
      ...
      if (!empty($products)) {
        foreach ($products as &$product) {
          $img_array[0] = ['img_name' => $product['img_name'], 'img_path' => $product['img_path']];
          
          $product['img_path'] = $this->fix_img_path($img_array);
